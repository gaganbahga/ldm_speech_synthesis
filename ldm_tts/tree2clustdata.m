function [clust_data,label_data,ldmModels] = tree2clustdata(tree,label_data)
% Tree to clustered data
%   Create a clustered Data struct array from the tree such that all the data corresponding
...
%       ldmModels: ldm Models which we get for free from clustering. But they will be autoregressive HMM
%       based as thats whats used for clustering
...
for segment_id = 1:length(seg_ids)
    ...
    for phn_id = 1:length(phones)
        ...
        [state_data,maxSplitLogL,bestQues,label_data,models] = get_clust_id(tree.(segment).(phone).node_ptr,...
            tree.(segment).(phone).data,segment_id,phone,label_data);
...
function [clust_data,maxSplitLogL,bestQues,label_data,ldmModels] = get_clust_id(node_ptr,data,segment_id,phone,label_data)
if isempty(node_ptr.leafNodes) % there were no long enough training sequences
    clust_data = struct();
    maxSplitLogL = struct();
    bestQues = struct();
    for i = 1:length(node_ptr.node_id)
        label_data(data(i).file_id).label{data(i).label_id} = label_data(data(i).file_id).label{data(i).label_id - 1};
    end
    ldmModels = [];
    return
end
...
for i = 1:length(node_id)
    if ~isfield(clust2state_map,node_id{i})
        ...
        i_clust = i_clust+1;
        clust_data.(new_state_id) = struct('label',{},'mgc',[],'root_ind',[],'file_id',[],'label_id',[]);
    end
